...
class NeuralSentimentTranslator:
    ...
    def translate_sentiment(self, audio_sentiment, noise):
        ...
        cur_noise = torch.tensor(truncated_noise_sample(batch_size=len(
            noise_vectors), truncation=noise)).to(config['device'])
        ...
        class_vectors, noise_vectors = class_vectors.cpu(
        ).detach().numpy(), noise_vectors.cpu().detach().numpy()
