...
def extract_features_from_file(path, n_mfcc=40, annotation_period=500, hop_length=512, sr=21504):
    ...
    tempo_features = librosa.feature.tempogram(
        y=y, sr=sr, hop_length=hop_length)
    chroma_features = librosa.feature.chroma_cens(
        y=y, sr=sr, hop_length=hop_length)
    spec_features = librosa.feature.melspectrogram(
        y=y, sr=sr, hop_length=hop_length)
    ...
    n_feats = int(
        features.shape[0] / feats_per_annotation_period) * feats_per_annotation_period
    features = features[:n_feats]
    spec_features = spec_features[:n_feats]

    features = features.reshape(-1,
                                feats_per_annotation_period * features.shape[1])
    features = np.float16(features)

    spec_features = spec_features.reshape(
        -1, feats_per_annotation_period, spec_features.shape[1])
    spec_features = np.mean(
        np.mean(np.float16(spec_features), axis=-1), axis=-1)
