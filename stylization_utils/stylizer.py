...
class Stylizer:
    def __init__(self, neutral_image=None, positive_image=None, negative_image=None, neutral_threshold=1,
                 stylization_model_path=join(
                     config['basedir'], 'models/stylization_model'),
    ...
    def _wct_stylization(self, content_image, style_image):
        ...
        std = torch.Tensor([0.229, 0.224, 0.225]).reshape(-1,
                                                          1, 1).to(config['device'])
        mean = torch.Tensor([0.485, 0.456, 0.406]
                            ).reshape(-1, 1, 1).to(config['device'])
...
def stylize_frames(images, stylizer, sentiments):
    ...
    for i in tqdm(range(len(images))):
        ...
        cur_img, cur_sentiment = stylizer.sentiment_stylization(
            cur_img, cur_sentiment)
